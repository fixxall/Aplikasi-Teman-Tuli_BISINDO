...
class Classifier {
    ...
    private static final String LABEL = "label2.txt";
    ...
    Classifier(Activity activity) throws IOException {
        ...
        Log.d(TAG, "asu: " + "done load mode");
    }

    void classifyMat(Mat mat) {
        if (tflite != null) {
            convertMatToByteBuffer(mat);
            Log.d(TAG, "asu: " + "run interface start");
            runInterface();
        }
    }
    ...
    private void runInterface() {
        if (imgData != null) {
            Log.d(TAG, "asu: " + "running start");
            tflite.run(imgData, probArray);
            Log.d(TAG, "asu: " + "running end");
        }
